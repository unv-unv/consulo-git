...
public class GitPushOperation {
  ...
  public GitPushResult execute() {
    ...
    try {
      ...
      for (int pushAttempt = 0; pushAttempt < MAX_PUSH_ATTEMPTS && !remainingRoots.isEmpty(); pushAttempt++, remainingRoots =
        getRejectedAndNotPushed(results)) {
        ...
        if (!result.rejected.isEmpty()) {
          ...
          if (myForce || pushingToNotTrackedBranch(result.rejected)) {
            ...
            updateSettings =
              showDialogAndGetExitCode(result.rejected.keySet(), updateSettings, rebaseOverMergeProblemDetected.booleanValue());
  ...
  @Nonnull
  private Collection<GitRepository> getRootsToUpdate(@Nonnull PushUpdateSettings updateSettings,
  ...
  private Map<GitRepository, GitPushRepoResult> push(@Nonnull List<GitRepository> repositories) {
    ...
    for (GitRepository repository : repositories) {
      ...
      if (resultWithOutput.isError()) {
        ...
        List<GitPushNativeResult> tagResults = filter(nativeResults,
  ...
  private void collectUpdatedFiles(@Nonnull UpdatedFiles updatedFiles,
  ...
  @Nullable
  private PushUpdateSettings showDialogAndGetExitCode(@Nonnull final Set<GitRepository> repositories,
                                                      @Nonnull final PushUpdateSettings initialSettings,
                                                      final boolean
                                                        rebaseOverMergeProblemDetected) {
    Ref<PushUpdateSettings> updateSettings = Ref.create();
    ApplicationManager.getApplication().invokeAndWait(() ->
                                                      {
                                                        GitRejectedPushUpdateDialog dialog = new GitRejectedPushUpdateDialog(myProject,
                                                                                                                             repositories,
                                                                                                                             initialSettings,
                                                                                                                             rebaseOverMergeProblemDetected);
                                                        ...
                                                        if (exitCode != DialogWrapper.CANCEL_EXIT_CODE) {
                                                          ...
                                                          updateSettings.set(new PushUpdateSettings(dialog.shouldUpdateAll(),
  ...
  @Nonnull
  protected GitUpdateResult update(@Nonnull Collection<GitRepository> rootsToUpdate,
                                   @Nonnull UpdateMethod updateMethod,
                                   boolean checkForRebaseOverMergeProblem) {
    GitUpdateResult updateResult = new GitUpdateProcess(myProject,
                                                        myProgressIndicator,
