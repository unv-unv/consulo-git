...
public class GitRebaser {
  ...
  public GitUpdateResult rebase(@Nonnull VirtualFile root,
    ...
    try (AccessToken ignored = DvcsUtil.workingTreeChangeStarted(myProject, "Rebase")) {
      ...
      return result.success() ? GitUpdateResult.SUCCESS : handleRebaseFailure(rebaseHandler,
                                                                              root,
   ...
   */
  public boolean reoderCommitsIfNeeded(@Nonnull final VirtualFile root,
                                       @Nonnull String parentCommit,
                                       @Nonnull List<String> olderCommits) throws VcsException {
  ...
  private static GitConflictResolver.Params makeParamsForRebaseConflict() {
    return new GitConflictResolver.Params().
                                             ...
                                             setErrorNotificationAdditionalDescription("Then you may <b>continue rebase</b>. <br/> " + "You also may <b>abort rebase</b> to restore the original branch and stop rebasing.");
  }

  public static class TrivialEditor extends GitInteractiveRebaseEditorHandler {
    public TrivialEditor(@Nonnull GitRebaseEditorService service,
