...
class GitHttpGuiAuthenticator implements GitHttpAuthenticator {
  ...
  public String askPassword(@Nonnull String url) {
    ...
    String password = passwordSafePromptDialog.askPassword(myTitle,
  ...
  @Nullable
  private AuthenticationData showAuthDialog(final String url, final String login) {
    final Ref<AuthenticationData> dialog = Ref.create();
  ...
  private String findPresetHttpUrl() {
    return ObjectUtil.chooseNotNull(ContainerUtil.find(myUrlsFromCommand, url -> {
