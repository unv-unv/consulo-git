...
public abstract class GitHandler {
  ...
  private void setupSshAuthenticator() throws IOException {
    ...
    boolean useHttpProxy = httpProxyManager.isHttpProxyEnabled() && !isSshUrlExcluded(httpProxyManager, ObjectUtil.assertNotNull(myUrls));
  ...
  protected static boolean isSshUrlExcluded(@Nonnull final HttpProxyManager httpProxyManager, @Nonnull Collection<String> urls) {
