...
public class GitCheckinEnvironment implements CheckinEnvironment {
  ...
  @Inject
  public GitCheckinEnvironment(@Nonnull Project project,
  ...
  public List<VcsException> commit(@Nonnull List<Change> changes,
    ...
    if (myNextCommitIsPushed != null && myNextCommitIsPushed.booleanValue() && exceptions.isEmpty()) {
      ...
      Application.get().invokeLater(() -> new VcsPushDialog(myProject,
                                                            preselectedRepositories,
                                                            GitBranchUtil.getCurrentRepository(myProject)).show(),
  ...
  private List<VcsException> commitWithCaseOnlyRename(@Nonnull Project project,
    ...
    Collection<Change> excludedStagedChanges = filter(stagedChanges,
                                                      change -> !caseOnlyRenames.contains(change) && !added.contains(getAfterPath(change)) && !removed
                                                        .contains(getBeforePath
                                                                    (change)));
        ...
        UIUtil.invokeAndWaitIfNeeded(new Runnable() {
          public void run() {
            ...
            SelectFilePathsDialog dialog =
              new SelectFilePathsDialog(project, files, message, null, "Commit All Files", CommonBundle.getCancelButtonText(), false);
  ...
  private void commitWithoutPaths(@Nonnull Project project,
                                  @Nonnull VirtualFile root,
                                  @Nonnull File messageFile,
  ...
  private void commit(@Nonnull Project project,
                      @Nonnull VirtualFile root,
                      @Nonnull Collection<FilePath> files,
                      @Nonnull File message) throws VcsException {
  ...
  private class GitCheckinOptions implements CheckinChangeListSpecificComponent, RefreshableOnComponent {
    ...
    private EditorTextField createTextField(@Nonnull Project project, @Nonnull List<String> list) {
      TextCompletionProvider completionProvider =
        new ValuesCompletionProvider.ValuesCompletionProviderDumbAware<>(new DefaultTextCompletionValueDescriptor.StringValueDescriptor(),
                                                                         list);
