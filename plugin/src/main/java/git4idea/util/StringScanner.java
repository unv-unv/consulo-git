...
public class StringScanner {
     ...
     */
    public StringScanner(@Nonnull String text) {
        myText = text;
        myPosition = 0;
    }
    ...
    public boolean isEol() {
        ...
        char ch = myText.charAt(myPosition);
        return ch == '\n' || ch == '\r';
    }
    ...
    public void nextLine() {
        ...
        if (hasMoreData()) {
            char ch = myText.charAt(myPosition++);
            if (hasMoreData()) {
                char ch2 = myText.charAt(myPosition);
                if (ch == '\n' && ch2 == '\r' || ch == '\r' && ch2 == '\n') {
                    myPosition++;
                }
            }
    ...
    public String boundedToken(char boundaryChar, boolean ignoreEol) {
        ...
        for (; myPosition < myText.length(); myPosition++) {
            char ch = myText.charAt(myPosition);
            if (ch == boundaryChar) {
                String rc = myText.substring(start, myPosition);
                myPosition++;
                return rc;
            }
            if (!ignoreEol && isEol()) {
    ...
    public void skipChars(int n) {
        if (n < 0) {
            throw new IllegalArgumentException("Amount of chars to skip must be non-negative: " + n);
        }
