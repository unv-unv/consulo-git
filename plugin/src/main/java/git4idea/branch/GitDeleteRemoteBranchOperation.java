...
class GitDeleteRemoteBranchOperation extends GitBranchOperation {
  ...
  public GitDeleteRemoteBranchOperation(@Nonnull Project project,
                                        @Nonnull Git git,
  ...
  private boolean doDeleteRemote(@Nonnull String branchName, @Nonnull Collection<GitRepository> repositories) {
    ...
    if (!result.totalSuccess()) {
      VcsNotifier.getInstance(myProject)
                 .notifyError("Failed to delete remote branch " + branchName, result.getErrorOutputWithReposIndication());
    }
