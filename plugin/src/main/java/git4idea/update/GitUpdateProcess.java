...
public class GitUpdateProcess
{
	...
	private GitUpdateResult updateImpl(@Nonnull UpdateMethod updateMethod)
	{
		...
		new GitPreservingProcess(myProject, myGit, myRootsToSave, "Update", "Remote", GitVcsSettings.getInstance(myProject).updateChangesPolicy(), myProgressIndicator, () ->
		{
	...
	private Map<GitRepository, GitUpdater> defineUpdaters(@Nonnull UpdateMethod updateMethod, @Nonnull Map<VirtualFile, GitBranchPair> trackedBranches) throws VcsException
	{
		...
		for(GitRepository repository : myRepositories)
		{
			...
			GitUpdater updater = GitUpdater.getUpdater(myProject, myGit, branchAndTracked, repository, myProgressIndicator, myUpdatedFiles, updateMethod);
	...
	private Map<VirtualFile, GitBranchPair> checkTrackedBranchesConfiguration()
	{
		...
		for(GitRepository repository : myRepositories)
		{
			...
			if(branch == null)
			{
				...
				notifyImportantError(myProject, "Can't update: no current branch", "You are in 'detached HEAD' state, which means that you're not on any branch" + mention(repository) + "<br/>" +
	...
	private boolean areUnmergedFiles()
	{
		...
		return !new GitMergeCommittingConflictResolver(myProject, myGit, myMerger, getRootsFromRepositories(myRepositories), params, false).merge();
	}
