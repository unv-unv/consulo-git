...
public class GitUpdateProcess {
    ...
    new GitPreservingProcess(myProject,
                             ...
                             () ->
                             {
                               ...
                               try {
                                 ...
                                 notifyImportantError(myProject,
  ...
  @Nonnull
  private Map<GitRepository, GitUpdater> defineUpdaters(@Nonnull UpdateMethod updateMethod,
                                                        @Nonnull Map<VirtualFile, GitBranchPair> trackedBranches) throws VcsException {
  ...
  private static String recommendSetupTrackingCommand(@Nonnull GitRepository repository, @Nonnull String branchName) {
    return String.format(GitVersionSpecialty.KNOWS_SET_UPSTREAM_TO.existsIn(repository.getVcs()
                                                                                      .getVersion()) ? "git branch --set-upstream-to=origin/%1$s %1$s" : "git branch --set-upstream %1$s " +
  ...
  private boolean areUnmergedFiles() {
    ...
    return !new GitMergeCommittingConflictResolver(myProject,
                                                   myGit,
                                                   myMerger,
