...
public class GitSkippedCommits extends PanelWithActionsAndCloseButton {
    ...
    public GitSkippedCommits(
        ...
        for (Map.Entry<VirtualFile, List<GitRebaseUtils.CommitInfo>> e : skippedCommits.entrySet()) {
            ...
            if (missed > 0) {
                vcsRoot.add(new DefaultMutableTreeNode(
                    "The " + missed + " commit(s) were not parsed due to unsupported rebase directory format"
                ));
            }
        }
    ...
    private DefaultTreeCellRenderer createTreeCellRenderer() {
        return new DefaultTreeCellRenderer() {
            ...
            public Component getTreeCellRendererComponent(
                ...
                Object o = value instanceof DefaultMutableTreeNode treeNode ? treeNode.getUserObject() : null;
    ...
    protected void addActionsTo(DefaultActionGroup group) {
        ...
        ShowAllAffectedGenericAction showCommit =
            (ShowAllAffectedGenericAction) ActionManager.getInstance().getAction(VcsActions.ACTION_SHOW_ALL_AFFECTED);
