	...
	private static class ShowLogInDialogTask extends Task.Backgroundable
	{
		...
		public void onSuccess()
		{
			if(!myVersion.isNull() && !myProject.isDisposed())
			{
				...
				WindowWrapper window = new WindowWrapperBuilder(WindowWrapper.Mode.FRAME, content).setProject(myProject).setTitle("Git Log").setPreferredFocusedComponent(content)
						.setDimensionServiceKey(GitShowExternalLogAction.class.getName()).build();
