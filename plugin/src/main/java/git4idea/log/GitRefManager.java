...
public class GitRefManager implements VcsLogRefManager {
  ...
  @Nullable
  private static SimpleRefGroup createTrackedGroup(@Nonnull GitRepository repository,
    ...
    private boolean isTracked(@Nonnull final VcsRef ref, final boolean remoteBranch) {
      ...
      return ContainerUtil.exists(repo.getBranchTrackInfos(), new Condition<GitBranchTrackInfo>() {
        ...
        public boolean value(GitBranchTrackInfo info) {
          return remoteBranch ? info.getRemoteBranch().getNameForLocalOperations().equals(ref.getName()) : info.getLocalBranch()
                                                                                                               .getName()
                                                                                                               .equals(ref.getName());
        }
      });
