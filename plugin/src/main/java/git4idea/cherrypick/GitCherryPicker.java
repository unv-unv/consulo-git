...
public class GitCherryPicker extends VcsCherryPicker {
  ...
  @Nonnull
  private static String toString(@Nonnull final Map<GitRepository, List<VcsFullCommitDetails>> commitsInRoots) {
    return StringUtil.join(commitsInRoots.keySet(), new Function<GitRepository, String>() {
      @Override
      public String apply(@Nonnull GitRepository repository) {
        String commits = StringUtil.join(commitsInRoots.get(repository), new Function<VcsFullCommitDetails, String>() {
          @Override
          public String apply(VcsFullCommitDetails details) {
  ...
  private boolean cherryPick(@Nonnull GitRepository repository,
    ...
    for (VcsFullCommitDetails commit : commits) {
      ...
      GitCommandResult result = myGit.cherryPick(repository,
      ...
      if (result.success()) {
        if (autoCommit) {
          ...
          boolean committed = updateChangeListManagerShowCommitDialogAndRemoveChangeListOnSuccess(repository,
                                                                                                  commitWrapper,
                                                                                                  successfulCommits,
                                                                                                  alreadyPicked);
          if (!committed) {
        ...
        boolean mergeCompleted =
          new CherryPickConflictResolver(myProject, myGit, repository.getRoot(), commit.getId().asString(), VcsUserUtil
            .getShortPresentation(commit.getAuthor()),
        ...
        String description =
          commitDetails(commitWrapper) + "<br/>Some untracked working tree files would be overwritten by cherry-pick.<br/>" + "Please move, remove or add them before you " +
            "can cherry-pick. <a href='view'>View them</a>";
  ...
  private boolean updateChangeListManagerShowCommitDialogAndRemoveChangeListOnSuccess(@Nonnull GitRepository repository,
  ...
  private void notifyConflictWarning(@Nonnull GitRepository repository,
                                     @Nonnull GitCommitWrapper commit,
                                     @Nonnull List<GitCommitWrapper> successfulCommits) {
    NotificationListener resolveLinkListener = new ResolveLinkListener(myProject,
                                                                       myGit,
                                                                       repository.getRoot(),
                                                                       commit.getCommit().getId().toShortString(),
                                                                       VcsUserUtil.getShortPresentation(
                                                                         commit
                                                                           .getCommit().getAuthor()),
                                                                       commit.getSubject());
    String description =
  ...
  @Nullable
  private CherryPickData updateChangeListManager(@Nonnull final VcsFullCommitDetails commit) {
  ...
  @Nullable
  private LocalChangeList createChangeListAfterUpdate(@Nonnull final VcsFullCommitDetails commit,
                                                      @Nonnull final Collection<FilePath> paths,
  ...
  private boolean showCommitDialogAndWaitForCommit(@Nonnull final GitRepository repository,
                                                   @Nonnull final GitCommitWrapper commit,
                                                   @Nonnull final LocalChangeList changeList,
                                                   @Nonnull final String commitMessage) {
    ...
    ApplicationManager.getApplication().invokeAndWait(new Runnable() {
      ...
      public void run() {
        try {
          ...
          boolean commitNotCancelled =
            AbstractVcsHelper.getInstance(myProject).commitChanges(changes, changeList, commitMessage, new CommitResultHandler() {
              @Override
              public void onSuccess(@Nonnull String commitMessage) {
  ...
  private void notifyError(@Nonnull String content,
  ...
  private void notifyResult(@Nonnull List<GitCommitWrapper> successfulCommits, @Nonnull List<GitCommitWrapper> alreadyPicked) {
    if (alreadyPicked.isEmpty()) {
      ...
      String title =
        String.format("Cherry-picked %d commits from %d", successfulCommits.size(), successfulCommits.size() + alreadyPicked.size());
      ...
      VcsNotifier.getInstance(myProject)
                 .notifyImportantWarning("Nothing to cherry-pick", formAlreadyPickedDescription(alreadyPicked, false));
    }
  }
    ...
    for (VirtualFile root : commits.keySet()) {
      ...
      for (Hash commit : commits.get(root)) {
        ...
        if (currentBranch != null && containingBranches != null && containingBranches.contains(currentBranch.getName())) {
          ...
          return String.format("The current branch already contains %s the selected %s",
                               commitsNum > 1 ? "one of" : "",
  ...
  private static class CherryPickConflictResolver extends GitConflictResolver {
    ...
    public CherryPickConflictResolver(@Nonnull Project project,
                                      @Nonnull Git git,
  ...
  private static class ResolveLinkListener implements NotificationListener {
    ...
    public ResolveLinkListener(@Nonnull Project project,
                               @Nonnull Git git,
                               @Nonnull VirtualFile root,
                               @Nonnull String commitHash,
                               @Nonnull String commitAuthor,
  ...
  private static class CherryPickMergeDialogCustomizer extends MergeDialogCustomizer {
    ...
    public String getMultipleFileMergeDescription(@Nonnull Collection<VirtualFile> files) {
      return "<html>Conflicts during cherry-picking commit <code>" + myCommitHash + "</code> made by " + myCommitAuthor + "<br/>" + "<code>\"" + myCommitMessage + "\"</code></html>";
    }
