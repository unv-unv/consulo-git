...
public class GitRefNameValidatorTest
{
	...
	public Object[][] createValidData()
	{
		return new Object[][]{
				...
				{
						"HIERARCHY_2",
						"user/branch/sub_branch"
				},
				{
						"BEGINS_WITH_SLASH",
						"/branch"
				},
				// actual branch name will be with trimmed slash
				{
						"NON_CONS_DOTS",
						"complex.branch.name"
				}
	...
	private static Object[][] populateWithIllegalChars(String[] illegalChars, Function<String, String> toString)
	{
		...
		{
			data[i] = new Object[]{
					toString.fun(illegalChars[i]),
					"bra" + illegalChars[i] + "nch"
			};
		}
		return data;
	}
	...
	{
		Assert.assertTrue("Should be valid", GitRefNameValidator.getInstance().checkInput(branchName));
		Assert.assertTrue("Should be valid", GitRefNameValidator.getInstance().canClose(branchName));
	}

	private static void assertInvalid(String branchName)
	{
		Assert.assertFalse("Should be invalid", GitRefNameValidator.getInstance().checkInput(branchName));
		Assert.assertFalse("Should be invalid", GitRefNameValidator.getInstance().canClose(branchName));
	}
}