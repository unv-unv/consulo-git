...
public class GitHistoryUtilsTest extends GitTest {
    ...
    public void setUp(Method testMethod) throws Exception {
        ...
        myRepo.refresh();
        File bIOFile = new File(dir.getPath(), "b.txt");
        ...
        for (int i = revisions.length - 1, j = 0; i >= 0; i--, j++) {
            ...
            GitTestRevision revision = new GitTestRevision(
                details[0],
                details[1],
                parents,
                commitMessages[i],
    ...
    private TestCommit move(String file, File dir, String message) throws Exception {
        String NAME = "PostHighlightingPass.java";
    ...
    private String toReadable(List<TestCommit> commits) {
        ...
        for (TestCommit commit : commits) {
            ...
            sb.append(String.format(
                "%s  %-" + maxSubjectLength + "s  %s%n",
                getShortHash(commit.getHash()),
    ...
    public void testGetCurrentRevisionInOtherBranch() throws Exception {
        ...
        myRepo.addCommit();
        String[] output = myRepo.log("--pretty=%H#%at", "-n1").trim().split("#");
    ...
    public void testGetLastRevisionForExistingFile() throws Exception {
        ItemLatestState state = GitHistoryUtils.getLastRevision(myProject, bfilePath);
        assertTrue(state.isItemExists());
        GitRevisionNumber revisionNumber = (GitRevisionNumber) state.getNumber();
        assertEquals(revisionNumber.getRev(), myRevisions.get(0).myHash);
        assertEquals(revisionNumber.getTimestamp(), myRevisions.get(0).myDate);
    }
    ...
    public void testOnlyHashesHistory() throws Exception {
        List<Pair<SHAHash, Date>> history = GitHistoryUtils.onlyHashesHistory(myProject, bfilePath, myRepo.getVFRootDir());
    ...
    public void testHashesWithParents() throws Exception {
        int expectedSize = myRevisionsAfterRename.size();
        ...
        for (Iterator hit = hashesWithParents.iterator(), myIt = myRevisionsAfterRename.iterator(); hit.hasNext(); ) {
            ...
            List<AbstractHash> parents = chpp.getParents();
    ...
    private static void assertEqualRevisions(GitFileRevision actual, GitTestRevision expected) throws IOException {
        ...
        assertEquals(
            ...
            convertWhitespacesToSpacesAndRemoveDoubles(expected.myCommitMessage)
        );
    ...
    private static void assertCommitEqualToTestRevision(GitCommit commit, GitTestRevision expected) throws IOException {
        ...
        assertEquals(
            convertWhitespacesToSpacesAndRemoveDoubles(commit.getDescription()),
            convertWhitespacesToSpacesAndRemoveDoubles(expected.myCommitMessage)
        );
    ...
    private static void assertCommitsEqualToTestRevisions(
        Collection<GitCommit> actualCommits,
        Collection<GitTestRevision> expectedRevisions
    ) throws IOException {
