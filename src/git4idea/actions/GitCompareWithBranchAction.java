...
public class GitCompareWithBranchAction extends DumbAwareAction
{
	...
	private static boolean noBranchesToCompare(@NotNull GitRepository repository)
	{
		...
		if(repository.isOnBranch())
		{  // there are other branches to compare
	...
	private static class OnBranchChooseRunnable implements Runnable
	{
		...
		{
			GitUIUtil.notifyMessage(project, GitUtil.fileOrFolder(file) + " doesn't exist in branch", String.format("%s <code>%s</code> doesn't " +
					"exist in branch <code>%s</code>", GitUtil.fileOrFolder(file), file.getPresentableUrl(), branchToCompare), false, null);
		}
	}
