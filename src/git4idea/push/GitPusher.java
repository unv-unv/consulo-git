...
public final class GitPusher
{
	...
	private static void runPushInBackground(
	...
	@NotNull
	private Map<GitRepository, List<GitBranchPair>> prepareRepositoriesAndBranchesToPush(@NotNull Map<GitRepository,
			GitPushSpec> pushSpecs) throws VcsException
	{
	...
	@NotNull
	private GitCommitsByBranch collectsCommitsToPush(
	...
	@NotNull
	private List<GitCommit> collectCommitsToPush(
			@NotNull GitRepository repository,
			@NotNull String source,
			@NotNull String destination) throws VcsException
	{
	...
	private GitPushResult tryPushAndGetResult(@NotNull GitPushInfo pushInfo)
	{
		...
		for(GitRepository repository : commits.getRepositories())
		{
			...
			{  // don't push repositories where there is nothing to push. Note that when a branch is created,
			// several recent commits are stored in the pushInfo.
				continue;
			}
			...
			{ // don't proceed if user has cancelled or couldn't login
				break;
			}
		}
	...
	@NotNull
	private GitPushRepoResult pushRepository(
			@NotNull GitPushInfo pushInfo,
	...
	@NotNull
	private static GitPushRepoResult getResultFromRejectedPush(
			@NotNull GitCommitsByRepoAndBranch commits,
			@NotNull GitRepository repository,
			@NotNull GitSimplePushResult simplePushResult)
	{
		...
		{
			// The rejectedDetector detected rejected push of the branch which had nothing to push (but is behind the upstream). We are not counting
			// it.
	...
	@NotNull
	private GitSimplePushResult pushAndGetSimpleResult(
			@NotNull GitRepository repository,
			@NotNull GitPushSpec pushSpec,
			@NotNull GitCommitsByBranch commitsByBranch)
	{
			...
			@NotNull GitRemoteBranch dest)
	{
		if(!branchTrackingInfoIsSet(repository, source))
		{
			...
			try
			{
				...
				GitConfigUtil.setValue(project, root, "branch." + branchName + ".merge", GitBranch.REFS_HEADS_PREFIX + dest
			...
			{
				LOG.error(String.format("Couldn't set up tracking for source branch %s, target branch %s, remote %s in root %s", source, dest,
						remote, repository), e);
	...
	@NotNull
	private static GitPushRepoResult successOrErrorRepoResult(
			@NotNull GitCommitsByRepoAndBranch commits,
			@NotNull GitRepository repository,
	...
	private static boolean branchInRejected(@NotNull GitBranch branch, @NotNull Collection<String> rejectedBranches)
	{
		...
		{
			if(rejectedBranch.equals(branchName) || (rejectedBranch.startsWith(REFS_HEADS) && rejectedBranch.substring(REFS_HEADS.length()).equals
					(branchName)))
			{
				return true;
			}
	...
	private void handleResult(
			@NotNull GitPushInfo pushInfo,
			@NotNull GitPushResult result,
			@Nullable GitPushResult previousResult,
			...
			int pushAttempt)
	{
		...
		else
		{
			...
			if(pushAttempt <= MAX_PUSH_ATTEMPTS && !rejectedPushesForCurrentBranch.isEmpty())
			{
				...
				LOG.info(String.format("Rejected pushes for current branches: %n%s%nUpdate settings: %s", rejectedPushesForCurrentBranch,
						updateSettings));
				...
				{
					Collection<GitRepository> repositoriesToUpdate = getRootsToUpdate(rejectedPushesForCurrentBranch,
							updateSettings.shouldUpdateAllRoots());
	...
	private void saveUpdateSettings(@NotNull UpdateSettings updateSettings)
	{
		...
		{ // null if user has pressed cancel
	...
	{
		GitUpdateProcess.UpdateMethod um = updateMethod == UpdateMethod.MERGE ? GitUpdateProcess.UpdateMethod.MERGE : GitUpdateProcess.UpdateMethod
				.REBASE;
		...
		else if(updateResult == GitUpdateResult.SUCCESS_WITH_RESOLVED_CONFLICTS || updateResult == GitUpdateResult.INCOMPLETE)
		{
			...
			{
				description = "Push has been cancelled, because not all conflicts were resolved during update.<br/>" + "Resolve the conflicts and " +
						"invoke push again.";
			}
			else
			{
