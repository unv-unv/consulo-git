...
public class GitCherryPicker
{
	...
	private boolean cherryPick(
			@NotNull GitRepository repository, @NotNull List<VcsFullCommitDetails> commits, @NotNull List<GitCommitWrapper> successfulCommits)
	{
		for(VcsFullCommitDetails commit : commits)
		{
			...
			GitUntrackedFilesOverwrittenByOperationDetector untrackedFilesDetector = new GitUntrackedFilesOverwrittenByOperationDetector(repository
					.getRoot());
			GitCommandResult result = myGit.cherryPick(repository, commit.getHash().asString(), myAutoCommit, conflictDetector,
			...
			else if(conflictDetector.hasHappened())
			{
				boolean mergeCompleted = new CherryPickConflictResolver(myProject, myGit, myPlatformFacade, repository.getRoot(),
						commit.getHash().asString(), commit.getAuthor().getName(), commit.getSubject()).merge();
			...
			else if(untrackedFilesDetector.wasMessageDetected())
			{
				...
				UntrackedFilesNotifier.notifyUntrackedFilesOverwrittenBy(myProject, untrackedFilesDetector.getFiles(), "cherry-pick", description);
				return false;
			}
	...
	private boolean updateChangeListManagerShowCommitDialogAndRemoveChangeListOnSuccess(
			@NotNull GitRepository repository, @NotNull GitCommitWrapper commit, @NotNull List<GitCommitWrapper> successfulCommits)
	{
			...
			@NotNull GitRepository repository, @NotNull GitCommitWrapper commit, @NotNull List<GitCommitWrapper> successfulCommits)
	{
		NotificationListener resolveLinkListener = new ResolveLinkListener(myProject, myGit, myPlatformFacade, repository.getRoot(),
				commit.getCommit().getHash().toShortString(), commit.getCommit().getAuthor().getName(), commit.getSubject());
	...
	private LocalChangeList createChangeListAfterUpdate(
			@NotNull final VcsFullCommitDetails commit, @NotNull final Collection<FilePath> paths, @NotNull final String commitMessage)
	{
	...
	private boolean showCommitDialogAndWaitForCommit(
			...
			@NotNull final GitCommitWrapper commit,
			@NotNull final LocalChangeList changeList,
	...
	private void notifyError(
			@NotNull String content, @NotNull GitCommitWrapper failedCommit, @NotNull List<GitCommitWrapper> successfulCommits)
	{
