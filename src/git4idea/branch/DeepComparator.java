...
public class DeepComparator implements Disposable
{
	...
	private class MyTask extends Task.Backgroundable
	{
		...
		@NotNull
		private final TIntHashSet myNonPickedCommits = new TIntHashSet();
		@Nullable
		...
		public void run(@NotNull ProgressIndicator indicator)
		{
			try
			{
				for(Map.Entry<GitRepository, GitBranch> entry : myRepositoriesWithCurrentBranches.entrySet())
				{
					...
					myNonPickedCommits.addAll(getNonPickedCommitsFromGit(myProject, repo.getRoot(), myProvider, currentBranch.getName(),
							myComparedBranch).toArray());
				}
			}
